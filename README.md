...
A production-style streaming data pipeline that ingests real-time traffic sensor events and applies data quality rules on the **Medallion Architecture** (Bronze → Silver → Gold).
...
| Metastore | Apache Hive 3.1.3 + PostgreSQL |
...
```bash
cd producer && python traffic_producer.py
```
...
| Issue | Handling |
|-------|----------|
| Corrupt JSON | → Quarantine table |
| Null mandatory fields (event_id, vehicle_id) | → Quarantine table |
| Invalid categorical values | → Nulled out |
| Out-of-range numeric values | → Clipped to valid range |
| Wrong datatypes (e.g. speed = "FAST") | → Cast attempt, null on failure |
| Invalid coordinates | → Clipped to valid lat/lon range |
